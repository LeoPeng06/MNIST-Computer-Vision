...
from torchvision.datasets import MNIST #import
...
class CNN(nn.Module) :
    def __init__(self, lr, epochs, batch_size, num_classes = 10):
        ...
        self.fcl = nn.Linear(input_dims, self.num_classes)
    ...
    def forward(self, batch_data):
        ...
        classes = self.fcl(batch_data)

        return classes
    def get_data(self): 
        mnist_train_data = MNIST('mnist', train = True, download = True, transform = ToTensor())

        self.train_data_loader = T.utils.data.DataLoader(mnist_train_data,batch_size=self.batch_size, shuffle= True, num_workers= 4) 
    ...
    def _train(self):
        ...
        for i in range(self.epochs):
            ...
            for j, (input, label) in enumerate(self.train_data_loader):
                ...
                loss.backward()
                self.optimizer.step()
            print('finish epoch' ' loss %.3f' % ep_loss, 'acc %.3f' %np.mean(ep_acc))
            self.loss_history.append(ep_acc)
